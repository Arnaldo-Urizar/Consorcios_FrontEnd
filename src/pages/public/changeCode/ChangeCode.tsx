...
const ChangeCode: React.FC = () => {
    ...
    const validatePassword = (pass1:string, pass2:string)=>{
        ...
        if(pass1 === pass2 && pass1.length >= minLength && hasNumbers.test(pass1)){
            return true;
        }else{
            return false;
        }
                ...
                <div className={styleLogin.wrapper}>
                    {!codeChanged ? (
                        <form onSubmit={handleChangeCodeSubmit} className={styles.form}>
                            ...
                            <button type="submit" className={styleLogin.btn} disabled={loading} >{loading ? "Cargando..." : "Cambiar Contraseña"}</button>
                        </form>
                    ):
                     ( 
                        <Alert
